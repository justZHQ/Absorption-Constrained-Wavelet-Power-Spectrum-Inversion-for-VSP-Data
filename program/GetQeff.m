function [Qeff,Q_by_Qeff,SpectrumRatio]=GetQeff(type,K,BegNum,Bandf,df,ampx,Time,Reference)
Reference=ampx(:,1);
TraT=Time;
dm=length(ampx(1,:));
Qeff=[];
...
Teff=[];
for i=1:dm-K
    TravelTime=TravelTime+TraT(i);
    Teff=[Teff;TravelTime];
    SpectrumRatio_i=log(ampx(:,i+K))-log(Reference);
    SpectrumRatio=[SpectrumRatio,SpectrumRatio_i];
    BandAbsSrcf=Reference;
...
Q_by_Qeff=[Qeff(1)];
for i=2:dm-K
    Q_by_Qeff=[Q_by_Qeff;1./(Teff(i)/Qeff(i)-Teff(i-1)/Qeff(i-1))];
end
Q_by_Qeff=1./Q_by_Qeff/2;  